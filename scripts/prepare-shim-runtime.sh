...
# the app bundle. This is an install-time step; starting the shim requires no
# dependency downloads, while requests to remote model providers still need a
# network connection.
set -eu
...
prepare_runtime() {
    ...
    if [ -x "$python" ] && "$python" -c "import aiohttp, importlib.metadata; raise SystemExit(0 if aiohttp.__version__ == '$AIOHTTP_VERSION' and importlib.metadata.version('github-copilot-sdk') == '$COPILOT_SDK_VERSION' else 1)" >/dev/null 2>&1; then
        return
    fi
...
# Keep generated bytecode out of SwiftPM resources and npm release archives.
find "$SHIM" -type d -name __pycache__ -prune -exec rm -rf {} +
find "$SHIM" -type f \( -name '*.pyc' -o -name '*.pyo' \) -delete
