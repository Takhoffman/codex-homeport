...
To scope the shim to a non-default Codex home, pass `--codex-home` or set
`CODEX_HOME` for the shim process:

```bash
codex-shim --codex-home ~/.codex-homes/shim-lab enable
codex-shim --codex-home ~/.codex-homes/shim-lab app .
```

This writes the managed provider block to that home's `config.toml` and reads
ChatGPT passthrough auth/model cache from that same home. Your main
...
Launchers that need a separate Desktop bundle can prepare a patched copy
instead of modifying `/Applications/Codex.app` in place:

```bash
codex-shim prepare-app \
  --destination "$HOME/Applications/Codex Shim.app"

codex-shim prepare-app \
  --destination "$HOME/Applications/Homeport Codex.app" \
  --display-name "Homeport Codex" \
  --bundle-name "Homeport Codex" \
  --bundle-id-suffix ".homeport"

codex-shim app-status --destination "$HOME/Applications/Homeport Codex.app"
```

`prepare-app` still applies the Codex Desktop ASAR patch, but the copy,
metadata, stale-source check, and optional branding are generic. Use
`--no-brand` if another launcher owns the copied app's bundle name and
identifier.
...
codex-shim prepare-app       prepare a patched Codex Desktop app copy
codex-shim app-status        inspect a prepared app copy and patch state
...
- `--codex-home <path>`: scopes Codex config/auth/model cache reads and writes.

`patch-app`, `prepare-app`, `app-status`, and `restore-app` are macOS-only and
do not use `--settings`. `patch-app` targets `/Applications/Codex.app` by
default but also accepts `--app-bundle`; `restore-app` remains the in-place
restore helper for the default patched Codex app.
