...
DEFAULT_CODEX_AUTH = DEFAULT_CODEX_HOME / "auth.json"
DEFAULT_CODEX_MODELS_CACHE = DEFAULT_CODEX_HOME / "models_cache.json"
...
def configure_codex_home(codex_home: Path | str | None) -> Path:
    """Set process-local Codex home paths used by passthrough helpers."""
    global DEFAULT_CODEX_HOME, DEFAULT_CODEX_AUTH, DEFAULT_CODEX_MODELS_CACHE
    home = Path(codex_home).expanduser() if codex_home is not None else _default_codex_home()
    DEFAULT_CODEX_HOME = home
    DEFAULT_CODEX_AUTH = home / "auth.json"
    DEFAULT_CODEX_MODELS_CACHE = home / "models_cache.json"
    return home
            ...
            models.append(
                ShimModel(
                    ...
                    reasoning_levels=_reasoning_levels(
                        _field(row, "reasoning_levels", "reasoningLevels", "supported_reasoning_levels", "supportedReasoningLevels")
                    ),
                    default_reasoning_level=_reasoning_level_or_none(
                        _field(row, "default_reasoning_level", "defaultReasoningLevel")
                    ),
...
def _reasoning_level_or_none(value: Any) -> str | None:
    if value in (None, ""):
        return None
...
def _reasoning_levels(value: Any) -> tuple[str, ...]:
    ...
    for item in raw_items:
        raw = item.get("effort") if isinstance(item, dict) else item
        if raw in (None, ""):
            continue
