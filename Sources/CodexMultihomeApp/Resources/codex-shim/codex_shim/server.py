...
def configure_codex_home(codex_home: Path | str | None) -> Path:
    """Set process-local Codex home paths for the shim server."""
    global CODEX_CONFIG_PATH, DEFAULT_CODEX_AUTH
...
class ResponsesStreamState:
    ...
    def __init__(self, model: str, tool_types: dict[str, str] | None = None):
        ...
        # Map chat-completions tool name -> original Responses tool metadata so
        # we can emit the right output item type and namespace.
...
def _build_tool_types(body: dict[str, Any]) -> dict[str, Any]:
    """Build a map chat tool name -> original Responses tool metadata.

    Codex Desktop emits native tools like `{"type": "apply_patch"}` and MCP tools
    as namespace wrappers like `{"type": "namespace", "name": "mcp__computer_use",
    "tools": [...]}`. When we translate those into chat-completions `function`
    tools, the original type and namespace are lost. We preserve them here so
    the Responses translators can emit the correct output item type and route
    MCP/plugin calls back through Codex Desktop.
    """
    ...
    for tool in body.get("tools") or []:
        ...
        if tool_type == "namespace":
            ...
            for nested in tool.get("tools") or []:
                if not isinstance(nested, dict):
                    continue
                fn = nested.get("function")
                if isinstance(fn, dict) and fn.get("name"):
                    nested_name = str(fn["name"]).strip()
                elif nested.get("name"):
                    nested_name = str(nested["name"]).strip()
                else:
                    continue
                clean = namespace_chat_tool_name(namespace, nested_name)
                tool_types[clean] = {
                    "type": tool_type,
                    "namespace": namespace,
                    "name": nested_name,
        ...
        if clean:
            namespace_info = _flat_namespace_tool_info(clean, tool)
            tool_types[clean] = namespace_info or tool_type
    return tool_types


def _flat_namespace_tool_info(clean_name: str, tool: dict[str, Any]) -> dict[str, str] | None:
    """Recover MCP namespace metadata after Responses tools are flattened.

    The Responses -> Chat translator exposes namespace tools to BYOK chat
    providers as ordinary function names such as
    ``mcp__computer_use__get_app_state``.  When the upstream model calls that
    function, Codex Desktop needs the original namespace/name pair back so it
    can route the call to the MCP server instead of treating it as a plain
    function tool.
    """
    if not clean_name.startswith("mcp__") or "__" not in clean_name[5:]:
        return None
    ...
    match = re.search(r"Namespace:\s*([A-Za-z0-9_-]+)\.\s*Tool:\s*([A-Za-z0-9_-]+)\.", description)
    if match:
        namespace, nested_name = match.group(1), match.group(2)
        if namespace_chat_tool_name(namespace, nested_name) == clean_name:
            return {"type": "namespace", "namespace": namespace, "name": nested_name}

    parts = clean_name.split("__")
    if len(parts) >= 3 and parts[0] == "mcp":
        namespace = "__".join(parts[:2])
        nested_name = "__".join(parts[2:])
        return {"type": "namespace", "namespace": namespace, "name": nested_name}
    return None


def _tool_info(value: Any) -> dict[str, str]:
    if isinstance(value, dict):
        return {str(k): str(v) for k, v in value.items() if v is not None}
    return {"type": str(value or "")}
