...
from .app_bundle import (
    AppBranding,
    app_asar_hash,
    app_patch_backup_dir,
    prepare_app_copy,
    prepared_app_is_stale,
    prepared_app_status_payload,
    write_prepared_app_metadata,
)
from .catalog import _toml_escape, codex_config_overrides, write_catalog, write_config, write_models_cache
...
from .settings import (
    ...
    configure_codex_home as configure_settings_codex_home,
)
...
CODEX_CONFIG_PATH = DEFAULT_CODEX_HOME / "config.toml"
CODEX_MODELS_CACHE_PATH = DEFAULT_CODEX_HOME / "models_cache.json"
CODEX_CONFIG_BACKUP_PATH = RUNTIME_DIR / (
    "config.toml.before-codex-shim."
    + hashlib.sha1(str(DEFAULT_CODEX_HOME).encode()).hexdigest()[:12]
)
...
SHIM_CODEX_APP = Path.home() / "Applications" / "Codex Shim.app"
...
BROWSER_PEER_AUTH_NEEDLE = re.compile(
    r"function (?P<name>\w+)\(\)\{if\(process\.platform!==`darwin`\)return\(\)=>\(\{authorized:!0\}\);"
    r"let (?P<metadata>\w+)=(?P<build>\w+)\.i\.readFromPackageMetadata\(\),"
    r"(?P<enabled>\w+)=(?P=metadata)!=null&&(?P=build)\.i\.shouldIncludeBrowserUsePeerAuthorization"
)
BROWSER_PEER_AUTH_REPLACEMENT = (
    r"function \g<name>(){return()=>({authorized:!0});"
    r"let \g<metadata>=\g<build>.i.readFromPackageMetadata(),"
    r"\g<enabled>=\g<metadata>!=null&&\g<build>.i.shouldIncludeBrowserUsePeerAuthorization"
)
BROWSER_PEER_AUTH_APPLIED = re.compile(
    r"function \w+\(\)\{return\(\)=>\(\{authorized:!0\}\);"
    r"let \w+=\w+\.i\.readFromPackageMetadata\(\),\w+=\w+!=null&&\w+\.i\.shouldIncludeBrowserUsePeerAuthorization"
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Codex home to read/write instead of CODEX_HOME or ~/.codex.",
    )
    ...
    patch_app_parser = sub.add_parser("patch-app", help="Patch Codex Desktop picker/sidebar handling for custom shim models.")
    patch_app_parser.add_argument("--app-bundle", type=Path, default=None, help="Patch this app bundle instead of Codex.app.")
    sub.add_parser("restore-app", help="Restore Codex Desktop app.asar from the pre-patch backup.")
    prepare_app_parser = sub.add_parser(
        "prepare-app",
        help="Create and patch a prepared Codex app copy without touching /Applications/Codex.app.",
    )
    prepare_app_parser.add_argument("--source", type=Path, default=SYSTEM_CODEX_APP)
    prepare_app_parser.add_argument("--destination", type=Path, default=SHIM_CODEX_APP)
    prepare_app_parser.add_argument("--replace", action="store_true", help="Re-copy the destination before patching.")
    prepare_app_parser.add_argument("--display-name", default="Codex Shim", help="Visible app display name for the prepared copy.")
    prepare_app_parser.add_argument("--bundle-name", default=None, help="Bundle name for the prepared copy; defaults to display name.")
    prepare_app_parser.add_argument("--bundle-id-suffix", default=".shim", help="Suffix appended to the copied app bundle identifier.")
    prepare_app_parser.add_argument("--no-brand", action="store_true", help="Do not rewrite copied app bundle name or identifier.")
    app_status_parser = sub.add_parser("app-status", help="Inspect the prepared Codex app copy and patch state.")
    app_status_parser.add_argument("--source", type=Path, default=SYSTEM_CODEX_APP)
    app_status_parser.add_argument("--destination", type=Path, default=SHIM_CODEX_APP)
    app_status_parser.add_argument("--json", action="store_true", help="Emit machine-readable JSON.")
    ...
    app_parser.add_argument("--app-bundle", type=Path, default=None, help="Launch this app bundle for Desktop.")
    app_parser.add_argument("--user-data-dir", type=Path, default=None, help="Pass an isolated Desktop profile directory.")
    app_parser.add_argument("path", nargs="?", default=".")

    args = parser.parse_args(argv)
    if args.codex_home is not None:
        configure_codex_home(args.codex_home)
    elif os.environ.get("CODEX_HOME", "").strip():
        configure_codex_home(None)
    ...
    if args.command == "patch-app":
        return patch_codex_app(args.app_bundle)
    if args.command == "restore-app":
        return restore_codex_app_bundle()
    if args.command == "prepare-app":
        return prepare_shim_codex_app(
            args.source,
            args.destination,
            replace=args.replace,
            display_name=args.display_name,
            bundle_name=args.bundle_name,
            bundle_id_suffix=args.bundle_id_suffix,
            brand=not args.no_brand,
        )
    if args.command == "app-status":
        return shim_codex_app_status(args.source, args.destination, json_output=args.json)
    ...
    if args.command == "app":
        ...
        ensure_started(args.settings, args.port)
        install_codex_config(args.settings, args.port, args.model_slug)
        exec_codex_app(args.settings, args.port, args.path, app_bundle=args.app_bundle, user_data_dir=args.user_data_dir)
        return 0
    return 2
...
def configure_codex_home(codex_home: Path | str | None) -> Path:
    """Set process-local Codex home paths for CLI commands."""
    global CODEX_CONFIG_PATH, CODEX_MODELS_CACHE_PATH, CODEX_CONFIG_BACKUP_PATH, DEFAULT_CODEX_AUTH
    home = configure_settings_codex_home(codex_home)
    CODEX_CONFIG_PATH = home / "config.toml"
    CODEX_MODELS_CACHE_PATH = home / "models_cache.json"
    CODEX_CONFIG_BACKUP_PATH = RUNTIME_DIR / (
        "config.toml.before-codex-shim."
        + hashlib.sha1(str(home).encode()).hexdigest()[:12]
    )
    DEFAULT_CODEX_AUTH = home / "auth.json"
    return home
...
def generate(settings_path: Path, port: int, default_slug: str | None = None) -> None:
    models = _load_models(settings_path)
    try:
        resolved_default_slug = default_slug or default_model_slug(models)
    except ValueError as exc:
        raise SystemExit(str(exc)) from exc
    router_config = router_module.load_router_config(Path(settings_path).expanduser())
    write_catalog(models, CATALOG_PATH, router_config=router_config, default_slug=resolved_default_slug)
...
def install_codex_config(settings_path: Path, port: int, model_slug: str | None = None) -> None:
    ...
    write_catalog(models, CATALOG_PATH, router_config=router_config, default_slug=default_slug)
    write_models_cache(models, CODEX_MODELS_CACHE_PATH, router_config=router_config, default_slug=default_slug)
...
def exec_codex(settings_path: Path, port: int, codex_args: list[str]) -> None:
    ...
    env["CODEX_HOME"] = str(CODEX_CONFIG_PATH.parent)
    if os.name == "nt":
...
def exec_codex_app(
    ...
    *,
    ...
    env = _with_loopback_no_proxy(os.environ.copy())
    env["CODEX_HOME"] = str(CODEX_CONFIG_PATH.parent)
    codex_app = app_bundle.expanduser() if app_bundle is not None else prepared_codex_app_bundle()
    if codex_app is not None:
        args = ["/usr/bin/open", "-n", str(codex_app)]
        for key, value in env.items():
            if key in {"CODEX_HOME", "NO_PROXY", "no_proxy"}:
                args.extend(["--env", f"{key}={value}"])
        if user_data_dir is not None:
            args.extend(["--args", f"--user-data-dir={user_data_dir.expanduser()}"])
...
def patch_codex_app(codex_app: Path | None = None) -> int:
    if sys.platform != "darwin":
        print("patch-app is macOS-only; Windows MSIX Codex Desktop cannot be patched with this ASAR helper.", file=sys.stderr)
        return 1
    codex_app = codex_app.expanduser() if codex_app is not None else _codex_app_bundle_for_patch()
    return _patch_codex_app_bundle(codex_app, quit_first=codex_app in {SYSTEM_CODEX_APP, USER_CODEX_APP})


def prepare_shim_codex_app(
    source: Path,
    destination: Path,
    *,
    replace: bool = False,
    display_name: str = "Codex Shim",
    bundle_name: str | None = None,
    bundle_id_suffix: str = ".shim",
    brand: bool = True,
) -> int:
    if sys.platform != "darwin":
        print("prepare-app is macOS-only; Windows MSIX Codex Desktop cannot be copied with this helper.", file=sys.stderr)
        return 1
    source = source.expanduser()
    destination = destination.expanduser()
    branding = AppBranding(display_name, bundle_name, bundle_id_suffix) if brand else None
    return prepare_app_copy(
        source,
        destination,
        backup_dir=_app_patch_backup_dir(destination),
        replace=replace,
        branding=branding,
        patch_app_bundle=lambda app: _patch_codex_app_bundle(app, quit_first=False),
    )


def shim_codex_app_status(source: Path, destination: Path, *, json_output: bool = False) -> int:
    if sys.platform != "darwin":
        print("app-status is macOS-only.", file=sys.stderr)
        return 1
    source = source.expanduser()
    destination = destination.expanduser()
    payload = _prepared_codex_app_status_payload(source, destination)
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 0

    source_hash = _short_hash(payload.get("source", {}).get("app_asar_sha256"))
    destination_hash = _short_hash(payload.get("destination", {}).get("app_asar_sha256"))
    print(f"Source:      {source}")
    print(f"  exists:   {_bool_text(payload['source_exists'])}")
    print(f"  version:  {_bundle_version_text(payload.get('source', {}))}")
    print(f"  asar:     {source_hash or 'missing'}")
    print(f"Prepared copy: {destination}")
    print(f"  exists:   {_bool_text(payload['destination_exists'])}")
    print(f"  version:  {_bundle_version_text(payload.get('destination', {}))}")
    print(f"  asar:     {destination_hash or 'missing'}")
    print(f"  patched:  {_bool_text(payload['patched'])}")
    print(f"  stale:    {_bool_text(payload['stale'])}")
    return 0


def _prepared_codex_app_status_payload(source: Path, destination: Path) -> dict[str, object]:
    return prepared_app_status_payload(
        source,
        destination,
        backup_dir=_app_patch_backup_dir(destination),
        is_patched=_app_asar_is_patched,
    )


def _bundle_version_text(signature: object) -> str:
    if not isinstance(signature, dict):
        return "missing"
    info = signature.get("info")
...
def _prepared_app_is_stale(source: Path, destination: Path) -> bool:
    return prepared_app_is_stale(source, destination, backup_dir=_app_patch_backup_dir(destination))


def _write_prepared_app_metadata(source: Path, destination: Path) -> None:
    write_prepared_app_metadata(source, destination, backup_dir=_app_patch_backup_dir(destination))


def _patch_codex_app_bundle(codex_app: Path, *, quit_first: bool) -> int:
    app_asar = codex_app / "Contents/Resources/app.asar"
    info_plist = codex_app / "Contents/Info.plist"
    backup_dir = _app_patch_backup_dir(codex_app)
    backup = backup_dir / APP_ASAR_BACKUP_NAME
    info_backup = backup_dir / INFO_PLIST_BACKUP_NAME
    ...
    versioned_backup = backup_dir / f"app.asar.before-codex-shim-model-picker-patch.{app_asar_hash(app_asar)[:12]}"
    ...
    if quit_first:
        _quit_codex_app()
    workdir = backup_dir / "app-asar-work"
    if workdir.exists():
        import shutil
...
def _app_patch_backup_dir(codex_app: Path) -> Path:
    return app_patch_backup_dir(codex_app, runtime_dir=RUNTIME_DIR, root_apps=(SYSTEM_CODEX_APP, USER_CODEX_APP))
...
def _patch_codex_desktop_bundles(workdir: Path) -> bool | None:
    patches = [
        ...
        (
            ...
            [".vite/build/main-*.js"],
            BROWSER_PEER_AUTH_NEEDLE,
            BROWSER_PEER_AUTH_REPLACEMENT,
            BROWSER_PEER_AUTH_APPLIED,
            False,
        ),
    ]
    changed = False
    for label, globs, needle, replacement, applied, required in patches:
...
def _find_js_bundle(
    ...
    for pattern in globs:
        root = workdir if "/" in pattern else assets_dir
        if not root.exists():
            continue
        candidates.extend(p for p in sorted(root.glob(pattern)) if p not in candidates)
...
def prepared_codex_app_bundle() -> Path | None:
    candidates: list[Path] = []
    env_bundle = os.environ.get("CODEX_SHIM_APP_BUNDLE", "").strip()
    if env_bundle:
        candidates.append(Path(env_bundle).expanduser())
    candidates.append(SHIM_CODEX_APP)
    for codex_app in candidates:
        app_asar = codex_app / "Contents/Resources/app.asar"
        if app_asar.exists() and _app_asar_is_patched(app_asar):
            return codex_app
    return None
...
def shimmable_codex_app_bundle() -> Path | None:
    return prepared_codex_app_bundle()
...
def _app_asar_is_patched(app_asar: Path) -> bool:
    try:
        ...
        return False
    return MODEL_PICKER_APPLIED.search(text) is not None and MODEL_PICKER_NEEDLE.search(text) is None
