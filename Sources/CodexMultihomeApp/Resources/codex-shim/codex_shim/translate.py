...
def chat_completion_to_response(payload: dict[str, Any], requested_model: str, tool_types: dict[str, Any] | None = None) -> dict[str, Any]:
    choice = (payload.get("choices") or [{}])[0]
    message = choice.get("message") or {}
    ...
    for call in message.get("tool_calls") or []:
        fn = call.get("function") or {}
        name = fn.get("name", "")
        tool_info = _tool_info(tool_types.get(name, ""))
        ...
        if tool_info.get("namespace"):
            item["namespace"] = tool_info["namespace"]
        output.append(item)
    return {
    ...
    for item in value:
        ...
        if item_type in {"message", None} and "role" in item:
            ...
            if namespace and name:
                name = namespace_chat_tool_name(namespace, name)
...
def _responses_tools_to_chat_tools(tools: Any) -> list[dict[str, Any]]:
    ...
    for tool in tools:
        if isinstance(tool, dict) and tool.get("type") == "namespace":
            converted.extend(_responses_namespace_tools_to_chat_functions(tool))
        else:
...
def _responses_namespace_tools_to_chat_functions(tool: dict[str, Any]) -> list[dict[str, Any]]:
    namespace = str(tool.get("name") or "").strip()
    if not namespace or not isinstance(tool.get("tools"), list):
        return []
    converted: list[dict[str, Any]] = []
    for nested in tool["tools"]:
        if not isinstance(nested, dict):
            continue
        nested_name = _responses_tool_function_name(nested)
        if not nested_name:
            continue
        chat_name = namespace_chat_tool_name(namespace, nested_name)
        description = nested.get("description") or _native_tool_description(nested)
        converted.append(
            {
                "type": "function",
                "function": {
                    "name": chat_name,
                    "description": f"{description} Namespace: {namespace}. Tool: {nested_name}.",
                    "parameters": nested.get("parameters") or _native_tool_parameters(nested),
                },
            }
        )
    return converted
...
def namespace_chat_tool_name(namespace: str, name: str) -> str:
    raw = f"{namespace}__{name}"
    ...
    return f"{clean[:55].rstrip('_')}_{digest}"
