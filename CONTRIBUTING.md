...
Thanks for helping improve Codex Homeport.
...
Homeport manages Codex state paths. Changes should preserve these rules:

- Never remove `~/.codex`.
- Never call `launchctl setenv CODEX_HOME`.
- Temporary homes must be reviewable before deletion.
- Destructive cleanup must move files to Trash, stay opt-in, and stay scoped to managed Homeport paths.
